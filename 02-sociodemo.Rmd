...
makeTableData(sexAtBirth)[3:1,] |>
  ...
  kable_paper("striped", full_width = F) |>
  kable_styling() |>
  column_spec(column = 1:1, width_min = "9.8em") |>
  scroll_box(width = "100%")
```
...
ggplot(pi.allYears, aes(x = year, y = est, color = labels)) +
  ...
  scale_color_manual(values = c("#EA008B", "#FF416f", "#FF7457", "#FFA349", 
                                "#FFCF50")) +
  labs(x = "Year", y = "Burning Man participants (%)", color = "Personal income (USD)")
...
reside <- makePlotData("Where do you usually reside (when not in Black Rock City)?",
                       ...
                       labels = c("Within California", "In Canada",
                                  "Within Nevada", "Other",
                                  "Other location within U.S."),
...
ggplot(reside, aes(x = year, y = est, color = labels)) +
  ...
  scale_color_manual(values = c("#EA008B", "#BDA5AD", "#FF7457", 
                     "#A07200", "#FFCF50")) +
  labs(x = "Year", y = "Burning Man participants (%)", color = "Residence (not in BRC)")
...
ggplot(politicalParty, aes(x = year, y = est, color = labels)) +
  ...
  scale_color_manual(values = c("#EA008B", "#BDA5AD", "#FF7457", "#A07200", 
                                "#FFCF50", "#554149")) +
  labs(x = "Year", y = "Burning Man participants (%)", color = "US politcal party \n affiliation")
...
tableDat |>
  ...
  footnote(alphabet = c("Middle eastern was not an option in 2013-2016.",
                        ...
                        "In 2022 the format changed to allow users to select multiple options.")) |>
  scroll_box(width = "100%")
```
...
#                                       times = 8),
#                          labels = c("English", "Spanish", "French", "Russian",
#                                     "German", "Chinese", "Other"),
#                          labelOrder = 1:7,
#                          groups = c(7, 6, 7,
#                                     7, 1, 7,
#                                     3, 5, 7,
...
# ggplot(language[language$year == 2022,], aes(x = labels, y = est)) +
#   geom_bar(stat = "identity", fill = "#EA008B") +
#   scale_y_continuous(labels = percent) +
#   labs(x = "Language", y = "") +
#   theme_bw() +
...
ggplot(language2, aes(x = year, y = est, fill = labels)) +
  ...
  labs(x = "Year", y = "Burning Man participants (%)", fill = "Language")
...
ggplot(spirituality, aes(x = year, y = est, color = labels)) +
  ...
  labs(x = "Year", y = "Burning Man participants (%)", color = "Language")
...
# ggplot(religion22, aes(x = year, y = est, color = labels)) +
#   geom_line(linewidth = 1.5) +
#   geom_point(size = 2.25, color = "black") +
#   theme_bw(13) +
#   theme(panel.grid.minor = element_blank()) +
...
makeTableData(disability[-9,]) |>
  kbl(col.names = c(2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  column_spec(column = 1:2, width_min = "9.8em") |>
  kable_styling() |>
  scroll_box(width = "100%")
```
...
# allSexualOrientation$labels <- factor(allSexualOrientation$labels,
#                                       levels = c("Heterosexual or \n straight",
#                                                  "Bisexual or \n pansexual",
#                                                  "Bicurious or \n heteroflexible",
#                                                  "Gay, lesbian, or \n homosexual",
#                                                  "Asexual", "Demi-sexual",
#                                                  "Refuse labels", "Other"))
sexualOrientationTableData <- makeTableData(allSexualOrientation)
sexualOrientationTableData[c(6, 8), 1:7] <- "--"
sexualOrientationTableData |>
...
ggplot(partner, aes(x = year, y = est, color = labels)) +
  ...
  scale_color_manual(values = c("#EA008B", "#BDA5AD", "#FF7457", "#A07200", 
                                "#FFCF50", "#554149", "#F9F871")) +
  labs(x = "Year", y = "Burning Man participants (%)", color = "Do you have a partner?")
