...
ggplot(mpg[mpg$year == 2022,], aes(x = mpg, weight = weights)) +
  ...
  geom_segment(x = mpg.means$mpg[mpg.means$year == 2022], 
             ...
             linewidth = 1.1) +
  annotate("text", x = 45, y = 0.0418, label = "Average miles per gallon to Black Rock City (18.9 MPG)") +
  theme_bw(13) +
...
ggplot(design22$variables, aes(x = miles2, weight = weights)) +
  ...
  annotate("text", x = 2500, y = 0.0005, label = "Average miles driven (1190.4 miles)") +
  theme_bw(13) +
...
ggplot(route, aes(x = labels, y = est, fill = as.factor(year))) +
  ...
  labs(x = "Route to Black Rock City", y = "", fill = "") +
  theme_bw() +
  coord_flip()
...
makeTableData(route)[12:1,] |>
  kbl(col.names = c(2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  column_spec(column = 1:3, width_min = "9.5em") |>
  kable_styling() |>
  scroll_box(width = "100%")
```
...
bxbshuttle <- makePlotData("Did you ride the satellite shuttle that operates within Black Rock City?",
                        ...
                        designs = list(design16, design17, design18,
                                       design19, design22),
                        years = c(2016:2019, 2022),
                        levels = c(rep(list(c("idk", "no", "yes"))),
...
ggplot(bxbreason, aes(x = year, y = est, color = labels)) +
  ...
  scale_x_continuous(breaks = c(2014:2019, 2022), labels = c(2014:2019, 2022)) +
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = c("#EA008B", "#BDA5AD", "#FF7457", "#A07200", "#FFCF50")) +
  guides(color = "none") +
...
bxbprice <- makePlotData("Based on your level of satisfaction with the program, would you say that the price of a Burner Express Bus ticket was",
                        ...
                        designs = list(design14, design15, design16, 
                                       ...
                                       design19, design22),
                        years = c(2014:2019, 2022),
                        levels = c(rep(list(c("overprice", "right_price", "underpriced"))),
...
parks <- makePlotData("Did you visit any parks or recreation areas this year on your way to or from Black Rock City?",
                      varNameTable = varNameTable,
                      designs = c(design15, design16, design17, design18,
                                  design19, design22),
                      years = c(2015:2019, 2022),
                      levels = c(rep(list(c("No", "Not_sure", 
                                            "Yes_2places_or_more", 
                                            "Yes_one_place")), times = 5),
                                 list(c("No", "Not sure",
                                        "More than one place",
                                        "One place"))),
...
ggplot(parks, aes(x = year, y = est, color = labels)) +
  ...
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = c("#EA008B", "#BDA5AD", "#FF7457", "#A07200")) +
  labs(x = "", y = "", color = "Parks or recreation visited")
...
```{r}
publicLands <- makePlotData("On your trip toor from the Burning Man event, are you visiting (or did you visit) public lands within a 30-mile radius of Black Rock City for any of the following reasons?",
                            varNameTable = varNameTable,
...
makeTableData(publicLands) |>
  ...
  kable_paper("striped", full_width = F) |>
  kable_styling() |>
  column_spec(column = 1:5, width_min = "9.5em") |>
  scroll_box(width = "100%")
```
...
# qrDat <- data.frame(y = c(census22$spendToGoToBRC, census19$bmspending,
#                           census18$bmspending, census17$bmspending,
#                           census16$bmspending, census15$bmspending,
#                           census14$bmspending),
#                     x = c(rep(2022, times = nrow(census22)),
#                           rep(2019, times = nrow(census19)),
#                           rep(2018, times = nrow(census18)),
#                           rep(2017, times = nrow(census17)),
#                           rep(2016, times = nrow(census16)),
#                           rep(2015, times = nrow(census15)),
#                           rep(2014, times = nrow(census14))),
...
rownames(spendTableValues) <- c("1%", "25%", "50% (median)",
                                "75%", "90%", "99%")

makeTableData(spendTableValues) |>
  kbl(col.names = c(2015:2019, 2022),
      row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  kable_styling() |>
...
nvSpendTableDat <- apply(spendTableDat, 2, function(x){
  sapply(x, function(y){
