...
ggplot(volunteer[volunteer$year == 2022,], aes(x = labels, y = est)) +
  geom_bar(stat = "identity", fill = "#EA008B") +
  scale_y_continuous(labels = percent) +
  labs(x = "Non-profit or charity", y = "") +
  theme_bw() +
  coord_flip()
...
kbl(tableDat, col.names = c(2018:2019, 2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  column_spec(column = 1:4, width_min = "9.5em") |>
  kable_styling() |>
  scroll_box(width = "100%")
```
...
ggplot(donation, aes(x = labels, y = est, fill = as.factor(year))) +
  ...
  scale_y_continuous(labels = percent) +
  labs(x = "Non-profit or charity", y = "", fill = "Year") +
  theme_bw() +
  coord_flip()
...
ggplot(inspiredVolunteer, aes(x = year, y = est, fill = labels)) +
  ...
  labs(x = "", y = "", fill = "Inspired to volunteer after \n Black Rock City?") +
  theme_bw()
...
makeTableData(donation)[15:1,] |>
  kbl(col.names = c(2016, 2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  column_spec(column = 1:6, width_min = "9.5em") |>
  kable_styling() |>
  scroll_box(width = "100%")
...
ggplot(skillsUseful, aes(x = year, y = est, color = labels)) +
  ...
  scale_y_continuous(labels = percent) +
  scale_color_manual(values = c("#EA008B", "#FF416f", "#FF7457", "#FFA349")) +
  labs(x = "Year", y = "", color = "Skills useful in the \n default world")
...
makeTableData(skillsOutside) |>
  kbl(col.names = c(2017, 2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
...
ggplot(skillsImpact, aes(x = factor(year, levels = c("2022", "2019", "2018")),
  ...
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(labels = percent) +
  labs(x = "Non-profit or charity", y = "", fill = "Impact of these skills") +
  theme_bw() +
  scale_fill_manual(values = c("#EA008B", "#BDA5AD", "#FF7457"))

makeTableData(skillsImpact) |>
  kbl(col.names = c(2018, 2022), row.names = TRUE) |>
  kable_paper("striped", full_width = F) |>
  column_spec(column = 1:4, width_min = "9.5em") |>
  kable_styling() |>
  scroll_box(width = "100%")
